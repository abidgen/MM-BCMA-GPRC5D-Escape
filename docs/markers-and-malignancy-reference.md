...
**"How do you know those are plasma cells?"**
> Seven-class panel, but the plasma call specifically needs two independent axes —
> secretory program (`MZB1`+`XBP1`) *and* mature identity (`SDC1` or `TNFRSF17`). `CD38`
> is excluded from the identity axis because it's on activated T cells and progenitors.
> The boundary agrees with the source paper on 32,307 of 32,337 cells.

**"How do you separate malignant from normal plasma cells?"**
> Light-chain restriction by ratio, not presence — Ig is the most ambient-contaminated
> family in marrow. Then a patient-specific dominant-V refinement, thresholded inside the
> empirical gap between donors and patients. That gives 21,906 cells across 32 patients.

**"Why not use CNV?"**
> I tried. It failed its own healthy-donor control — one donor had a median plasma z
> above 3 and a 50.6% false-positive rate. I rejected the method before looking at a
> single disease sample, so it contributes no evidence in either direction. That's why I
> never say "subclone" about this cohort.

**"Isn't using TNFRSF17 for the plasma call circular?"**
> Partly, and it's disclosed rather than defended. `SDC1` was ruled not evaluable in the
> coverage analysis on exactly that basis; `TNFRSF17` carries the identical
> selection-dependence and is retained only because it's the frozen anchor. The predicate
> operated at cluster level and no cell was dropped for its own `TNFRSF17` zero — so it's
> a selection-dependence risk, not proof the BCMA estimate is invalid.

**"Why is `NKG7` not enough for an NK call?"**
> Because it's a cytotoxic *state*, shared with effector T cells. Calling NK from
> `NKG7`/`GNLY` alone over-called 22,132 cells in an earlier revision. Identity needs
> `KLRD1`/`KLRF1`/`NCAM1`/`FCGR3A`/`KLRC1`; the effector genes are reported as state.

---

*Sources: `src/mm_escape/config.py`, `src/mm_escape/malignant.py`,
`results/06_annotation/STAGE06_FINAL.md`, `results/07_malignant_plasma/STAGE07_DESIGN.md`,
`results/07_malignant_plasma/v_clone_membership/`,
`results/07_malignant_plasma/cnv_negative_result/CNV_FROZEN_NOT_EVALUABLE.md`.*
