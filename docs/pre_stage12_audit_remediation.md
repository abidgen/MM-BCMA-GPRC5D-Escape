...
> **Editorial note, added after the fact:** this remediation, and the re-audit that
> followed it (`docs/pre_stage12_codex_reaudit.md`), cleared Stage 12 to begin. Stage 12
> ran to completion on 2026-08-27 (`results/12_final_synthesis/`). This document is
> preserved verbatim as the historical remediation record.
...
| **MEDIUM — stale notebook/result names in README and pipeline doc** | `RESOLVED` | `README.md`: the stage table now names `07_malignant_plasma`, `08_dual_antigen_escape`, `09_bulk_validation`, `10_dn_coherence`, `11_immune_context` and the lettered arms `05b`, `08c`, `09b`, `11b`, with correct result directories. `mm_dual_antigen_escape_pipeline.md`: 10 stale notebook/result references corrected and a dated naming banner added at the top. The main project document: the two remaining stage-09/10 prose references corrected. | `grep` for the five planning-era names returns only the explanatory banner. | `mm_dual_antigen_escape_pipeline.md` remains a planning-era document; its banner says the main project document is current where they disagree. Only naming was corrected — no scientific prose was rewritten. |
| **MEDIUM — contradictory `jupytext --sync` instruction** | `RESOLVED` | The main project document (line 441) no longer says sync keeps the pair in lockstep; it now points at the notebook-pairing rule and mandates explicit `jupytext --to notebook` in the `.py → .ipynb` direction. | `grep -n jupytext` over that document — the forbidding rule and the review workflow now agree. | None. |
| **MEDIUM — source-of-truth contradiction (`results/*.csv` called the source of truth while ignored)** | `RESOLVED` | Replaced in both places (the main project document and `docs/stage-results.md`) with: *frozen scientific outputs are local/archived artifacts authenticated by the committed freeze manifest; committed production code and provenance metadata define how those artifacts were produced and verified.* Both now state that an ignored local file is not by itself durable provenance and outranks prose only once its hash matches. | `grep "source of truth"` in the main project document and `docs/stage-results.md`. | Full closure depends on H1's external deposit. |
...
0 robust-low**. This matches the main project document exactly: Stage 09 is
interpretation context, never an additional scoring axis.
...
3. **Bind Stage-12 semantics to H2** — the weak-discriminator wording is now binding in
   the main project document and in the function's own docstring.
